...
" Language:	Javascript with YUI and jQuery keywords
" Changes: Jose Elera and Dav Glass
"
" Original:	Mikolaj Machowski ( mikmach AT wp DOT pl )
...
function! javascriptcomplete#CompleteJS(findstart, base)
  if a:findstart
	...
	if shortcontext =~ '\.$'
        ...
        "YAHOO.util
        let yahoouprop = ['Dom', 'Event', 'CustomEvent', 'Anim']
	    call map(yahoouprop, 'v:val."."')
        let yahooutil = yahoouprop

        "YAHOO.util.Dom
        let yahooudmeth = ['addClass', 'batch', 'generateId', 'get', 'getClientHeight', 'getClientWidth', 'getDocumentHeight', 'getDocumentWidth',
                    \ 'getElementsBy', 'getElementsByClassName', 'getRegion', 'getStyle', 'getViewportHeight', 'getViewportWidth', 'getX', 'getXY',
                    \ 'getY', 'hasClass', 'inDocument', 'isAncestor', 'removeClass', 'replaceClass', 'setStyle', 'setX', 'setXY', 'setY']
		...
		"jQuery   {{{
		let jQueryprop = ['boxModel', 'browser', 'version']
		let jQuerymeth = ['ajaxComplete', 'ajaxError', 'ajaxSend', 'ajaxStart', 'ajaxStop', 'ajaxSuccess',
					\ 'ajax', 'get', 'getJSON', 'getScript', 'post', 'load',
					\ 'ajaxSetup', 'serialize', 'serializeArray',
					\ 'attr', 'removeAttr',
					\ 'addClass', 'removeClass', 'toggleClass',
					\ 'html', 'text', 'val',
					\ 'css', 'height', 'width', 'offset',
					\ 'noConflict', 'extend', 'fn', 'each', 'eq', 'index', 'length', 'size',
					\ 'hide', 'show', 'toggle', 'animate', 'dequeue', 'queue', 'stop', 'fadeIn', 'fadeTo', 'fadeOut', 'slideUp', 'slideDown', 'slideToggle', 'bind', 'one', 'trigger', 'triggerHandler', 'unbind', 'hover',
					\ 'blur', 'change', 'click', 'dblclick', 'error', 'focus', 'keydown', 'keypress', 'keyup', 'mousedown', 'mousemove', 'mouseout', 'mouseover', 'mouseup', 'resize', 'scroll', 'select', 'submit', 'unload', 'ready',
					\ 'param', 'data', 'removeData',
					\ 'clone', 'wrap', 'wrapAll', 'wrapInner', 'append', 'appendTo', 'prepend', 'prependTo', 'after', 'before', 'insertAfter', 'insertBefore', 'empty', 'remove', 'replaceAll', 'replaceWith',
					\ 'andSelf', 'end', 'eq', 'filter', 'hasClass', 'is', 'map', 'not', 'slice', 'add', 'children', 'contents', 'find', 'next', 'nextAll', 'parent', 'parents', 'prev', 'prevAll', 'siblings',
					\ 'each', 'extend', 'grep', 'inArray', 'makeArray', 'map', 'unique', 'trim', 'isFunction']
		call map(jQuerymeth, 'v:val."("')
		let jQuery = jQueryprop + jQuerymeth
		...
		elseif shortcontext =~ 'YAHOO\.$'
			let values = yahoo
		elseif shortcontext =~ 'YAHOO\.util\.$'
			let values = yahooutil
		elseif shortcontext =~ 'YAHOO\.util\.Dom\.$'
			let values = yahooutildom
		elseif shortcontext =~ 'YAHOO\.util\.Event\.$'
			let values = yahooutilevent
		elseif shortcontext =~ 'YAHOO\.util\.Anim\.$'
			let values = yahooutilanim
		elseif shortcontext =~ 'YAHOO\.Tools\.$'
			let values = yahootools
            ...
            let values += yahoo + yahooutil + yahooutildom + yahooutilevent + yahootools + yahooutilanim
	...
	let htmldom = ['YAHOO', 'document', 'anchor', 'area', 'base', 'body', 'event', 'form', 'frame', 'frameset', 'history', 'iframe', 'image', 'input', 'link', 'location', 'meta', 'navigator', 'object', 'option', 'screen', 'select', 'table', 'tableData', 'tableHeader', 'tableRow', 'textarea', 'window']
